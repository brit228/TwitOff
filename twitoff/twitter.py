...
def add_user(user_id):
    ...
    n_user = TwitterUser(id=t_user.id, name=t_user.screen_name,
                         newest_tweet_id=tweets[0].id)
    DB.session.add(n_user)
    for tweet in tweets:
        embedding = BASILICA.embed_sentence(tweets[0].full_text,
                                            model="twitter")
        n_tweet = Tweet(id=tweet.id, text=tweet.full_text, embedding=embedding,
                        user_id=tweet.author.id, user=n_user)
